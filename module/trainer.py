...
class LearningEnv:
    ...
    def __set_model__(
        ...
        **kwargs,
        ...
        if pretrained_model is not None:
            #据字符串self.model_name在模块M中获取一个属性或方法，并使用model_args作为关键字参数来创建一个model对象
    ...
    def get_dataloader(self, dataset_file, batch_size, num_worker, shuffle=True, contain_context=False):
        ...
        # print("*************line140",utterance_input_ids_t.shape)
        dataloader_params = {
            "dataset": dataset_,
        ...
        def get_pair_pad_idx(utterance_input_ids_batch, window_constraint=3, emotion_pred=None):
            ...
            for batch in check_pad_idx.view(-1, max_doc_len):
                ...
                for end_t in range(1, len(batch.nonzero()) + 1):
                    ...
                    #pair_window_idx[max(0, int((end_t - 1) * end_t / 2), int(end_t * (end_t + 1) / 2) - window_constraint):int(
                      #  end_t * (end_t + 1) / 2)] = 1
                    #pair_window_idx[max(0,int((end_t-1)*end_t)/2),int(end_t*(end_t+1)/2)-window_constraint,int(end_t*(end_t+1)/2)]=1 #int前面):
            ...
            for batch in check_pad_idx.view(-1, max_doc_len):
                ...
                for end_t in range(1, len(batch.nonzero()) + 1):
                    if emotion_pred is not None and emotion_pred[end_t - 1] == 6:
                        continue
                    pair_window_idx[
                        max(0, int(((end_t - 1) * end_t)/2), int(end_t * (end_t + 1) / 2) - window_constraint):int(end_t * (end_t + 1)/ 2)] = 1

                    #pair_window_idx[max(0, int((end_t - 1) * end_t) / 2), int(end_t * (end_t + 1) / 2) - window_constraint,int(end_t * (end_t + 1) / 2)] = 1#int前面是);
                check_pair_window_idx.append(pair_window_idx)
